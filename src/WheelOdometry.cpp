...
WheelOdometry::WheelOdometry(): Node("wheel_odometry"), robotinoKinematics(0.04f, 0.130f, 16.f) {
    ...
    joint_state_subscriber = this->create_subscription<sensor_msgs::msg::JointState>("/robot/joint_state", 10, 
...
void WheelOdometry::handle_wheel_pos_callback(const sensor_msgs::msg::JointState& msg) {
    if(use_position) {
        robot_position = robotinoKinematics.direct(static_cast<float>(msg.position[0]), 
                                                    static_cast<float>(msg.position[1]),
                                                    static_cast<float>(msg.position[2]));
    }
    else {
        robot_speed = robotinoKinematics.direct(static_cast<float>(msg.velocity[0]), 
        ...
        double dt_sec = msg.header.stamp.sec - prev_stamp.sec +
                        (static_cast<double>(msg.header.stamp.nanosec) -
                            static_cast<double>(prev_stamp.nanosec)) / 1000000000.0;
        robot_position[0] += std::cos(robot_position[2]) * robot_speed[0] * static_cast<float>(dt_sec) - 
                                std::sin(robot_position[2]) * robot_speed[1] * static_cast<float>(dt_sec);
        robot_position[1] += std::sin(robot_position[2]) * robot_speed[0] * static_cast<float>(dt_sec) + 
                                std::cos(robot_position[2]) * robot_speed[1] * static_cast<float>(dt_sec);
